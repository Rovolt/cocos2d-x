...
bool FTTextPainter::SetFont(Platform::String^ fontName , UINT nSize)
{
	...
	}else
	{
		...
		std::string fullPath = CCFileUtils::sharedFileUtils()->fullPathFromRelativePath(CCUnicodeToUtf8(fontName->Data()).c_str());
		Platform::String^ fullPathRefStr = ref new Platform::String(CCUtf8ToUnicode(fullPath.c_str()).c_str());
